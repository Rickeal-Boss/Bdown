...
## 待办：HEVC 的 hvc1 标签（已知兼容性问题，未修）

**问题**：B 站部分 HEVC 流的 `stsd` sample entry 写的是 `hev1`，而
QuickTime / Safari / iOS 相册**只认 `hvc1`**。我们合并时直接沿用源流的
sample entry，所以产物在这些播放器里可能**无法解码**（文件结构看起来完全正常）。

**为什么没修**：修法要在 `buildMergedMoov`（`src/core/mp4.js:502`）里把视频 trak
的 `hev1` 4CC 替换成 `hvc1`。虽然两者码流相同、只改标签，但它动的是**合并主路径**，
而本轮用户即将做真机验证 —— 在验证前引入这种改动不合适。

**什么时候做**：等真机验证后，若确认 HEVC 产物在 Apple 系播放器里确实有问题，
再单独修并加合成夹具测试（夹具已支持 `sampleEntry: 'hev1'`）。

## 本轮外部挖取的结论与边界（诚实记录）

派了三路成员并行深挖外部仓库，但实际拿到的**完整报告只有 qa-lead 的 mp4.js 审计**。
因此本轮落地的内容全部有**本地实测 / 逐行核对**依据：

| 落地项 | 依据 |
|---|---|
| 片段扫描：大 moof 被静默丢弃 | 合成夹具实测（moof 20088 > 16384 窗口，确实被丢）+ 两条独立来源确认 |
| durl 多段只下第一段 | 逐行核对代码 + 回归测试（产物 = 段1 拼接 段2） |
| NFO 控制字符 / 时区 / title 兜底 | 本地实测 + 评审交叉确认 |

**没能做到的**：与 yt-dlp / yutto 的**完整对照**。investigator 两轮标记完成
但都只回完成通知、没有正文，催过两次也无果。**这一项标注为未做，不假装做过。**
