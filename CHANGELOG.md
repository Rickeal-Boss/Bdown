...
> → 主理人交叉核实 → 4 路修复代理并行实现（每路**先复现失败用例再改代码**）→ 总控收口构建。
> 本轮不是加功能，是把 v1.4.24~26 三轮改动里**说谎的文案、名不副实的测试、
> 以及我自己引入的一个回归**一次性清干净。

### 🔴 P1：续传清单失效后重下 → 进度条卡住不动（v1.4.26 我自己引入的回归）

v1.4.26 为修真机「暂停→继续进度条倒退」加了 `nextDownloadedBytes` 单调不减。
**但我只防了「倒退」，没防「该清零时没清零」**：

1. `resumeTask` 恢复暂停任务时保留 `downloadedBytes`（60MB），续传场景下这是对的
2. 但清单**已失效**时（TTL 过期 / 大小不匹配 / `.part` 实际长度对不上），
   `openPartial` 判 `fresh` → `truncate(0)` → 本次实际从 0 重下
3. 新 sink 从 0 起，基线仍是 60MB → 单调不减把它永久保留 → 进度条**纹丝不动**

倒退看得见、会自己恢复；卡死看不出原因，用户只会以为"下载挂了"。
新增 `resolveProgressBase(prev, resumed)`：按「本次是否真续传」决定保留还是清零。
`merge` 走 `prepareStage` 能拿到 `resumed`；`durl / separate / audio` 本就不走续传，直接清零。

### 🟠 P1：仅音频 + 保存对话框 → 面板显示的名字与磁盘真实名字不一致

`createOutput` 在 `singleOutput + destination.kind==='file'` 时直接用用户句柄、
**忽略 `name`**，而 `finishOutput` push 的是引擎算出的名字。
用户把文件名手输成 `.m4a` 时 → **面板显示 `.flac`、磁盘是 `.m4a`**，
用户按面板去磁盘找文件找不到，会判定"扩展坏了"。
新增 `resolveOutputPath()`：仅该路径用 `out.handle.name`（真实磁盘名），其余路径不变。

### 🟠 P1：`audioOutputMeta` 子串匹配误判 FLAC

`mime.includes('flac')` 是子串匹配，B 站若返回带参数的 MIME
（`audio/mp4; codecs="flac"`，MP4 容器装 FLAC 编码）会被误判成 `.flac`，
扩展名与真实内容不符。改为**按 `;` 截断取主类型再精确比对**，
同时保留 `type === 'flac'` 兜底（B 站可能只给 type）。

### 🟠 P1：弹窗文案承诺「将下载完整视频」，实际任务会失败

`buildPlan` 里是 `if (!audio) throw`（无条件抛错），
而弹窗提示写的是「无法只下载音频，将下载完整视频」—— 实际**什么都下不了**。
改为诚实且可操作的提示，引导用户自己改用「合并为 MP4」。

### 🟠 P1：三处文案仍写「保存为 .m4a」

v1.4.25 起无损轨已存 `.flac`，但 `popup.js` 两处、`options.html` 一处没跟着改。
新增 `MODE_HINTS` 常量作为**单一真源**（原来同一份文案抄了两遍，必然漂移），
并**主动去掉「直接可播」这个无法保证的承诺** —— 无损路径至今未在真机验证过。

### 🟠 P1：`fetchExtras` 用了全局 settings，任务级 mode 失效

`run()` 有任务级 settings 覆盖（弹窗选的下载方式本次生效），
但 `fetchExtras` 是独立方法、内部 `const { settings } = this` 拿全局。
于是「全局 merge + 本次仅音频 + 开了 NFO」时仍会生成 NFO，
而仅音频产物是 `.m4a`/`.flac`，NFO 基名对不上。
新增 `resolveTaskSettings()` 供两处共用；顺带加**白名单校验** ——
storage 残留若给了 `'audioo'` 这类非法但 truthy 的值，会静默落回 merge 产出完整视频
（falsy 值反而安全）。

### 🟡 P2

- **仅音频直写路径缺 `truncate(0)`**：`FileHandleSink.open()` 用 `keepExistingData: true`
  （续传需要），新内容更短时尾部残留上一轮字节。仅音频不走续传，截断是安全的
- **弹窗「下载方式」区加「（本次生效）」标记**，设置页加对应说明
- **仅音频时清晰度区降透明度**（不禁用 —— 它仍参与文件名 `{qualityShort}`）
- **切换下载方式后清晰度列表的预计体积不刷新**（仍是 merge 口径）
- **README 漏「仅音频」**，且全文无 `.flac` 产物说明
- **`api.js` 双读 `mime_type` 兜底无断言**

### 测试：把「名不副实」的测试改诚实

`tools/test-resume-restart.mjs`（14 项）**对改动前的旧实现 100% 通过** ——
它只测 `resume-store`/sink，而 v1.4.24 的根因在 `engine` 的 `onResolvedSize`。
文件头却宣称「复现了暂停后从头下」。已改为诚实描述（写明它**不**完整覆盖那条路径），
并补上真正的根因断言 `[6]`：估算 size ≠ 探测 size 时，写进清单的必须是探测到的真实值。

`onResolvedSize` 此前在 `tools/` 中**零引用**，即本轮唯一根因修复没有任何断言保护，已补。

新增 `tools/lint-docs-consistency.mjs`（5 项）：锁住 README 与设置页文案不漂移，
含一条「不得出现『已真机验证』类措辞」的守护断言。

### 决定不修（记录）

- 切换模式后若清晰度列表未刷新，仅影响预计体积显示，不影响产物
- `lint-noundef` 的固有盲区：`return` 后写正则字面量会被当成除号，产生假阳性（已规避）

---

## [1.4.26] - 2026-09-21

### 🟡 真机验证后修：暂停 → 继续时进度条瞬时倒退

真机实测录屏（2026-09-21）：

| 时刻 | 进度 | 已下字节 |
|---|---|---|
| 22.360s | 62% | 44.26 MB（暂停前） |
| 30.621s | **80%** | 56.90 MB（继续后） |
| 31.093s | **68%** | **48.47 MB**（倒退了 8.43 MB） |
| 32.793s | 72% | 51.25 MB |
| 33.559s | 79% | 56.00 MB（再次暂停前） |
| 37.914s | **68%** | **48.47 MB**（又倒退到同一个值） |
| 38.425s | **86%** | 61.12 MB（最终追上） |

**数据没丢** —— 最终 86% 超过暂停前 79%，v1.4.24 的修复确实生效了。
但进度条两次精确倒退到 48.47 MB，用户看到的第一反应就是"续传又没生效"。

这是断点续传的**通病**：已下字节的「记账」与「真实落盘」之间存在时间差 ——
续传用的 sink 刚建好时 `sink.size` 还没把已下字节写回（`openPartial` 的回填
发生在 `fetchTo` 之前），此刻按 `sink.size` 算出的已下字节就会偏小。

修法：新增 `nextDownloadedBytes()`，已下字节**只许前进不许后退**。
宁可让进度条暂时偏保守，也绝不允许它倒退。抽成纯函数是为了能直接加断言
（`refreshProgress` 藏在 `run()` 的闭包里测不到）。

### 📋 真机验证结果（v1.4.24 / v1.4.25 的修复）

| 项 | 结果 |
|---|---|
| 暂停 → 继续（v1.4.24） | ✅ **生效**。段 1 完美（62% → 80%），最终 86% > 暂停前 79%，字节没丢 |
| 仅音频（普通 AAC） | ✅ **`.m4a` 产物正常**。无损场景未测到 —— 用户非大会员，Hi-Res 轨不会返回 |
| 弹窗 mode 不写回全局（v1.4.25 P0-3） | ✅ **生效**。同一标题的两个不同 mode 任务都成功完成 |

### ⚠️ 一处误判更正

上一版我报告「文件名出现重复分隔符 `..`」—— **这是我的误判**（VLM 读图精度问题）。
用户确认实际文件名是 `《…》…。_1080P · 80 · 1080P · AVC`，
即 `{title}` 末尾的中文句号后接模板分隔符 `_`，格式完全正常，无需修复。

### 测试

28 套件 / 全绿。`selftest-core` 71 → **78** 项，新增 `nextDownloadedBytes` 7 项。
已验证断言非空：用改动前的实现复现，会算出 48,470,000（正是真机实测值），新断言确实会失败。

## [1.4.25] - 2026-09-21

> 用户要求「加入只下载音频功能（只下载音频直接交付）」。
>
> 排查结论：**这个功能一直都在**（提交 `596ddf9` 就实现了，设置页、弹窗、引擎、
> 测试四层齐全），真正的问题是**产物不对**：默认配置下会把 FLAC 无损音轨
> 存成 `.m4a`，用户双击打不开 —— 所以体感上像"没有这个功能"。
> 本轮修的是缺陷，不是加功能。

### 🔴 P0-1：无损音轨被存成 `.m4a`（默认路径就会踩到）

`audioPreference` 默认 `'best'` → `pickAudioTrack` 会**优先选中** FLAC 无损轨，
而输出侧 `engine.js` 硬编码 `.m4a` + `audio/mp4`。

无论 B 站的 FLAC 是裸流还是 fMP4 封装，硬编码都是错的：
- 裸 FLAC → 扩展名不对，播放器按 `.m4a` 解析失败
- fMP4 封装 → 扩展名对，但 Chrome 不认 fLaC 编码，UI 承诺的"直接可播"落空

新增 `audioOutputMeta(track)`，由接口返回的 `mimeType` 驱动（与 yt-dlp 的
`ext = mimetype2ext(mimeType)` 同思路，从不硬编码）：
`flac → .flac / audio/flac`，杜比与普通 AAC → `.m4a / audio/mp4`。
同时修了「音视频分离」模式的同病（原来也是写死 `.audio.m4a`）。

### 🔴 P0-2：设置项「普通音轨」完全失效

`api.js` 的 rank 公式里，普通 AAC 恒为 **4**（`4 - min(3, floor((id-30200)/100))`，
30216/30232/30280 的 floor 都是 0），而 flac=3、dolby=2 —— 排序是**升序取第一个**，
于是选「普通音轨（最高码率）」反而优先拿到**杜比**，与文案完全相反，
非会员也躲不开（这两种轨只在有权限时返回，用户没法主动避开）。

改为：普通模式下把 flac/dolby 排到 99（不是直接过滤 —— 万一只有无损轨还能兜底）。

### 🔴 P0-3：弹窗选的「下载方式」被写回全局设置

`popup.js` 在点「开始下载」时把 `downloadMode` 一并 `saveSettings`。
后果：为**一次**需求选了「仅音频」，之后**所有**下载都静默变成仅音频，
设置页也被改掉 —— 用户会以为设置自己跑了。

这段代码上方早有注释警告过同类问题（`defaultQuality` 静默重置），
那次只修了清晰度，`downloadMode` 是漏网的。现在改为随任务传递
（`specs[].downloadMode`），引擎优先用它、不再触碰全局设置。

### 🟠 P1：仅音频 + 「每次询问保存位置」时，保存对话框强制 `.mp4`

`dashboard.js` 的 `showSaveFilePicker` 一律建议 `.mp4`、类型只给 `video/mp4`。
而「仅音频」是 singleOutput，引擎直接写进用户选的句柄 ——
于是磁盘上是 `xxx.mp4` 却只有音频，面板显示 `xxx.m4a`，两边对不上。
新增 `savePickerHint()` 按下载方式给建议名与类型过滤。

### 🟡 P2：文件名残留符号

仅音频时 `plan.codec` 填的是 `'—'`（真值）→ 文件名模板产出 `标题_—`；
`plan.quality` 是 0（假值）→ `{quality}` 被替换成空串 → `标题_`。
现在 codec 填音轨真实编码（`flac` / `aac`）。

### 测试

28 套件 / 全绿。`selftest-core` 63 → **71** 项：新增 `audioOutputMeta` 4 项、
`pickAudioTrack` 普通模式 3 项。已验证断言**非空** —— 用改动前的 rank 公式复现，
旧实现在「普通音轨」下会选中 dolby，新断言确实会失败。

### 顺带收掉的三项 P2

- **durl / 老视频下选「仅音频」会被静默忽略** —— 这类视频没有独立音轨，
  engine 会退回下载完整视频，用户选了「仅音频」却拿到 MP4 会以为坏了。
  现在弹窗提前提示「该视频没有独立音轨，无法只下载音频」。
- **弹窗「预计体积」不区分模式** —— 仅音频时仍把视频大小算进去，
  显示的预计值比实际产物大一个数量级。现已按模式计算。
- **`api.js` 只读 `a.mimeType`** —— B 站同时返回 `mimeType` 与 `mime_type` 且同值，
  只读一种的话，哪天服务端只给另一种，扩展名会**静默退回 .m4a**
  （无损轨因此变成打不开的文件）。已按 yt-dlp 的做法两种都读。

### 未处理（记录，不阻塞）

- 文件名模板在「变量为空时去掉分隔符」这一层没处理，`{quality}` 仍可能留下尾随下划线。
- 入口可发现性：「仅音频」在 360px 弹窗里和 3 个视频向选项挤在一行，
  产品官建议抽成紧邻「开始下载」的常驻快捷开关（未做）。

## [1.4.24] - 2026-09-20

### 🔴 修复：暂停后「继续」却从头下载（续传静默失效）

现象：开启断点续传后可以正常暂停，但点「继续」时进度从 0 开始，已下的字节全被丢弃。

**根因**：`fetchTo` 的 `total` 从未被**真实**大小修正。

`downloadRanged` 内部会先发 `Range: bytes=0-0` 探测精确大小并修正它自己的 `size`
（估算值来自 `bandwidth × duration / 8`，误差可达数个百分点），但 `fetchTo` 的 `total`
一直是最初传进来的估算值。于是：

```
暂停时写清单   → { size: 估算值 }     ← 错的
继续时 openPartial → 期望 { size: 探测过的真实值 }
canResume       → 两者不相等 → 判「大小不匹配」→ truncate(0) → 从头下载
```

`downloadRanged` 本来就提供了 `onResolvedSize` 回调，这一版把它接上了；
成功路径另外用 `result.size` 兜底（末尾分片的 `shortAtEof` 也会修正真实大小）。

**排查过程中差点误报**：第一版复现测试失败了 8 项，一度以为找到了 bug，
实际是测试的 mock 不完整 —— `ResumeStore.write` 用的是 `w.write(字符串)` 简写形态，
而 `read` 用的是 `file.text()`，我的假 OPFS 两处都没实现。
**测试失败不等于产品有问题，先确认测试本身站得住。**

### 新增诊断：清单用不上时会说明原因

`openPartial` 判定为"不能续传"时，若**确实存在清单**（首次下载没清单不告警，避免刷屏），
会打印 `续传清单不可用，本次将从头下载：<原因>`。

原因可能是「大小不匹配」「清单已过期」或「磁盘长度对不上」，三者修法完全不同 ——
以前只能靠猜，现在看 Console 就知道。

### 新增：`tools/test-resume-restart.mjs`（14 项）

用**内存版 OPFS** 跑完整链路：随机偏移写入会扩展文件长度（空洞补 0，
与 File System Access API 一致），覆盖"写入 → 写清单 → 重开 → 判定"全过程。

为什么必须有这一层：上一轮 `planResume` / `mismatchWithDisk` 的测试**全是纯函数**，
判据写错也照样全绿 —— 直到真机上出现"暂停后从头下"才暴露。
纯逻辑测试绕开了 OPFS 的真实语义，这里把它补回来。

覆盖：顺序下载后暂停 / 乱序完成（并发分片常态，有空洞）/ 只下一小片 /
对照组的三种"必须从头下"（大小不匹配、无清单、清单说下完但文件是空的）。
...
> 两件事：**给下载中心补上「暂停 / 继续」按钮**；**定位并修掉「下载后半段变慢」**。
>
> 关于暂停按钮：它**始终可见**。未开启断点续传时会弱化显示，点击后说明原因并
> 打开设置页 —— 早期版本在未开启时把按钮**隐藏**了，结果"用户反馈没有暂停按钮"
> 的体感一点没变，功能上线了却看不见。
>
> 关于「变慢」的关键结论：**不是 B 站服务端限速**。排障手核过 yt-dlp 官方
> `bilibili.py`，全文没有任何 CDN 限速 / 连接数 / 并发的记载；
> 中文检索命中的「DownKyi 动态线程调度」等说法均为 AI 生成的营销稿，无一手来源。
> 真正的结构性原因在我们自己的分片调度里（见下 B、C 两条）。
...
- **暂停 ≠ 取消**，唯一区别是清不清续传清单：`pause()` 只置意图标记 + abort，
  清单留在 OPFS；`cancel()` 会调 `discardResume()` 连 `.part` 一起删掉。
  （顺带修了一个反向隐患：先暂停再点取消时，`paused` 标记会被打回 false，
  否则「取消」会被当成「暂停」，清单被误留。）
- 继续时**复用上一轮的保存位置**，不重新弹文件选择器 —— 每次都重选的话
  「继续」比「取消重下」还麻烦，用户就不会用这个功能了。
  句柄授权失效（File System Access 跨会话不自动恢复）时才退回重选，并说明原因。
- 只在「下载中」提供暂停：`resolving` 还没有字节可留；`muxing` / `saving`
  中断会产出半截文件，只能取消。
- **没开断点续传时不显示「暂停」** —— 那时中断即丢弃，点了「继续」也是从头下，
  那是骗用户。显隐由 `data-resume` 交给 CSS，不用 `.hidden`
  （作者样式表的 `display:inline-flex` 优先级高于 UA 的 `[hidden]`，设了无效）。
- 工具栏新增「全部暂停」。

### 🔴 P0：已下完的轨被抹掉重下（续传等于没生效）

`fetchTo` 成功时原本 `resume.store.clear(key)` —— 把清单删掉，等于**忘了记
「这条轨已经下完」**。下次 `openPartial` 读不到清单 → `canResume` 判 false →
走 `truncate(0)` 把 `.part` 整个抹掉。

合并模式下必现：视频轨先下完、音频轨下到一半时暂停 → 继续时视频轨本该直接跳过，
却被重下整份。

改为写一条「区间 = 全量」的清单；`resume-store.js` 新增纯函数 `planResume()`，
**先判 `complete` 再判 `canResume`**（后者的契约是"已完成 < 总大小"才算可续，
下完的轨它一律判 false）。

### 🟠 P1：响应体读取没有任何超时 —— 「后半段变慢」的主因

`doFetch` 的 30s 超时只计到**响应头**。服务端一旦"先痛快回响应头、再慢慢滴灌"，
那个计时器早就清掉了，这一片会**无限期读下去**；其余分片早已下完，
总进度就卡在最后几个百分点 —— 用户感知到的"被限速"，绝大多数是这一片在拖。

新增 `readBody()`：逐块读，每收到一块续一次命，**15s 零字节即判定停滞**，
主动放弃并 `reader.cancel()`（不关的话连接一直挂着白占并发额度），
交给重试换连接。判定条件是"零字节"而非"速度低于某值"，避免误杀弱网下的正常大分片。

### 🟠 P1：一片失败就丢弃全部进度、单连接从头重下

`fetchTo` 出错后一律 `resetSink()` + `downloadSequential()`：
已下好的字节全部作废，然后**单连接**从头再下一遍 —— 8 倍降速。
在 80% 处触发一次，用户看到的就是"突然变慢"，而且看起来像网络抽风。

现在先拿 `doneRanges` 保留进度重试一次（不调 `resetSink`），仍失败才回退顺序下载。
`rangeIgnored`（服务器真不支持 Range）除外，那种情况必须重来。
配套改动：onProgress 里**无论有没有开续传都在内存累计 doneRanges**
（原来只有开启续传时才记，于是没开续传的用户必然从 0 重下）。

### 🟠 P1：8 路并发全打同一个地址

`backupUrls` 只当故障切换用，从不参与负载均衡 —— 8 路全挤在同一台 CDN 主机上
（HTTP/2 下更是共用一条 TCP 连接的拥塞窗口）。改为第 i 个 worker 从
`list[i % list.length]` 起跳。备用地址连续失败 2 次则**关掉轮换**：
`backupUrls` 的签参不保证完整，继续轮换只会给每个分片白搭几次失败尝试。

### 🟠 P1：sink 写入链会被单次失败永久毒化

`writeAt` 原先把 `.then()` 的结果**直接赋回 `_chain`**。一次写失败 →
...
**收尾补修（P2-3）**：`fetchTo` 两条**重试**路径原本传的是原始 `onProgress` 而非包装版本，
导致重试期间下的字节不进 `doneRanges`。已统一为共用的 `wrappedProgress`。
这是语义一致性修复 —— 当前无实际后果（重试成功即标记完成、失败即回落顺序下载），
但留着它，将来任何依赖 `doneRanges` 的逻辑都会漏掉重试那一段。

补了一条非空断言锁住「失败重试必须带着已下的进度」：1MB 分两片、第二片首轮失败，
断言第一片**只被请求过 1 次**、写入偏移不重复（响应里能看到
`保留进度重试一次（已下 524288 / 1048576 字节）`）。
...
安全官结论 🟡 有条件放行（无 P0），发布检查 🟢 Go。落地改动如下。

**🟠 P1：播放地址没有域名白名单（F-01）**

`api.js` 的 `httpsUrl()` 只做 `http://` → `https://` 替换，**没有任何域名校验**，
而 `base_url` / `backup_url` 直接来自 playurl 响应体 —— 响应里写什么我们就去请求什么。
PoC：喂 `backupUrl: ['http://evil.example.com/collect', 'http://169.254.169.254/...']`
两个都会被原样放进下载地址列表。

用户只授权了 manifest 里那 6 个 B 站域，代码却能打任意域 —— **权限声明与实际行为不符**。
`credentials: 'omit'` 挡住了最大的一块（B 站登录态不外泄），但真实 IP / 真实 UA /
`Origin: chrome-extension://<id>` 仍会外泄，对无鉴权的内网 CGI 也够用。

修法：新增 `MEDIA_HOST_SUFFIXES`（= B 站域 **+ `akamaized.net`**）与 `safeMediaUrl()`，
在 `api.js` 构造播放地址时过滤。

⚠️ **不能直接复用 `BILI_HOST_SUFFIXES`** —— 它不含 `akamaized.net`，而 manifest 的
`host_permissions` 与 DNR 规则 1 **都声明了它**。用错白名单会把合法的 akamai 备用 CDN
全滤掉，造成下载直接失败。已加断言同时锁住"必须放行 akamaized"与"必须挡下 evil/内网地址"。

**🟡 P2：顺序下载回退路径没有停滞检测（F-05）**

`downloadSequential`（服务器不支持 Range 时的兜底）是裸 `reader.read()`，
没有任何超时 —— 于是 `readBody` 专门要治的"滴灌"在这条路上原样存在，
而且它是**单连接**，挂住比分片路径更彻底。已抽出 `readChunk()` 供两条路径共用。

**🟡 P2：默认状态下看不见暂停按钮**

未开启断点续传时用 CSS 把「暂停」**隐藏**了，结果"用户最初反馈没有暂停按钮"的体感
一点没变。改为一律可见：未开启时半透明弱化，点击后 toast 说明原因并打开设置页。
（不用 `disabled` —— 禁用按钮点了没反馈，用户不知道为什么点不动。）

**🟡 P2：打包脚本的污染防护有洞（第二次踩同类坑）**

上次只防了"下划线开头的目录"（`_zv`），于是污染换了个形态又进来了：
两个临时探针脚本 `.sec-ping.mjs` / `.sec-verify.mjs` 被整份打进包里
（44 → 46 文件），而包内结构肉眼看不出异常。点开头的文件 `ls` 默认不显示，比 `_zv` 更难发现。
已补 `HIDDEN_RE` + 包内断言（双保险：源头排除 + 产物校验）。

顺带清掉 0 字节的 `node_diff.txt`（自 `f62075d` 起误提交）→ 包内 43 文件。

### 已讨论并**决定不修**的（附理由，避免下轮重复争论）

- **F-03「任务失败时清 .part」** —— 会变成**功能降级**：任务 60% 失败后点「重试」，
  `prepareStage` 的 key 由内容身份派生（不含 URL），现状是**从 60% 续**；
  加清理就变成从 0 重下。泄漏有界（TTL 7 天），真要治存储应加孤儿 .part 清扫器。
- **F-02「complete 分支补 TTL」** —— 曾加过随即撤销。磁盘长度校验已保证残缺清单
  不会被当进度用；加 TTL 会让"字节完整但超过 7 天"的轨白白整轨重下。
  用确定的体验损失去防不确定的陈旧风险，不划算。**partial 分支的 TTL 照常生效**，
  两种判定各自成立（已加断言锁住，防止有人"顺手统一"）。
- **F-04「readBody 峰值内存 ×2」** —— 峰值约 256MB（16MB × 8），需改成流式落盘，
  改动面较大，留到下一轮。
...
| 套件 | 项数 | 覆盖 |
|---|---|---|
| `tools/test-stall-rotation.mjs` | 31 | 停滞检测 / 取消贯穿 body 读取 / 多地址分摊 / 轮换边界与自动关闭 / 监听不堆积 / 写入链不毒化 / **顺序下载回退路径的停滞检测** |
| `tools/test-resume-pause-guard.mjs` | 26 | complete 必须被 `.part` 真实长度挡下 / `toRecord` 必须带 resumeKeys / 无上一轮位置必须问用户 / **失败重试必须带进度** / 暂停与取消的分界 |
| `tools/test-security.mjs` | 41 → **50** | 合法 akamai CDN 必须放行 / evil 域与内网地址必须挡下 / `file://` / 协议相对 URL |
| `tools/test-release-gate.mjs` | **9（新增）** | 见下 |
| `planResume`（test-resume-store） | 38 → 41 | 含"刻意不校验 TTL"的决策锁定 |

新测试全部同步登记进 `validate.yml`（`lint-ci-coverage.mjs` 当场抓到未登记的并已修好）。

### 新增：发布门禁 `tools/test-release-gate.mjs`

站在**产物侧**独立复核打包结果，不信任上游的排除规则 —— 因为排除规则本身也可能被改坏：
包内文件数必须等于基线（43，可用 `BDOWN_PKG_BASELINE` 显式更新）、不得有点开头隐藏文件、
不得有 `_` 开头临时目录、`manifest.json` / `_locales` / `src` / `rules` 必备结构在位、
顶层不得出现源码目录之外的杂物。用最小 ZIP 解析器读 central directory，不引第三方依赖。

在 `validate.yml` 里排在「打包扩展」**之后** —— 门禁要读产物，排在打包之前时
`dist/` 还不存在，会直接失败并把后续步骤全部 skip（**CI #55 就是这么红的**：
门禁被重复注册了两份，其中一份位置错误，已删除）。

### CI #55 曾红（已修）

新增门禁时 `validate.yml` 里出现了**两条** `test-release-gate`：一条在打包之后（正确），
一条在打包之前（此时 `dist/` 不存在 → 门禁失败 → 后续步骤全被 skip）。

两个教训：
1. 依赖前序步骤产物的步骤，**光"注册了"不够，位置错了照样红**；
2. 同一脚本被重复注册**不会报错**（`lint-ci-coverage` 只检查"是否被引用"），
   重复只能靠人工核对 —— 加完 grep 一下步骤名是否只出现一次。

### 真机验证建议

1. 设置里打开「断点续传」→ 下一个 1080P 视频 → 下到一半点**暂停** → 点**继续**：
   进度条应从断点接着走（不回退到 0），且总耗时明显少于重下。
2. Console 里若出现 `读取停滞：15000ms 内没有收到任何字节` 且随后自动恢复，
   说明停滞检测正在生效（旧版会永久卡住）。
3. 关掉「断点续传」开关 → 下载中不应出现「暂停」按钮。
...
### 🔴 P1：打包产物被临时目录污染（44 文件 → 88 文件）

某次校验命令被中断，临时解压目录 `_zv` 残留，下一次打包把它**整份**塞进了扩展包：
文件数 44 → 88，体积 468KB → 940KB，包内出现一整套重复的 `_zv/src/...`。
这种污染在 zip 里很难肉眼发现，却会让商店提交和真实安装都出问题。

**修复两层防护**（`tools/package.mjs`）：
1. `walk` 排除所有下划线开头的目录 —— 用否定顺序环视 `/^_(?!locales)/` 保留 `_locales`
   （扩展国际化目录，必需，**不能**一刀切排除）
2. 打包后断言：若仍有 `_` 开头（非 `_locales`）的顶层目录则**直接失败**并提示
   "多半是上次脚本中断留下的残留"；同时反向断言 `_locales` 必须存在

已验证：清理后重新打包为 44 文件 / 469.7 KB，包内顶层结构正确、`_locales` 仍在、
关键文件逐字节一致。

- **`maxParallelTasks` 未在"逐个点开始"路径生效**：`startTask` 完全没检查 `runningCount`，
  于是逐个点「开始」时想跑几个跑几个，而该设置只对「全部开始」生效 —— 用户设了 2 却同时
  跑 5 个，会以为设置坏了。现已检查上限并给出明确提示（ask 模式下无法自动排队，
  因为每个任务都要用户选保存位置）。

### 决定**不**修的三项（说明理由，避免下轮重复纠结）

- **弹幕条数上限**：10 万条弹幕生成 ASS 约 2s 同步阻塞。加截断虽能消除卡顿，
  但会**静默丢失用户的弹幕数据** —— 用"可能掉数据"换 2 秒延迟不划算，且这 2s 发生在
  本就很慢的保存阶段。若将来改为分片/异步渲染再处理。
- **续传 `.part` 无 TTL 自动清理**：续传默认**关闭**，只有主动开启的用户才会产生 `.part`；
  且下载中心已有「清理临时文件」按钮。收益有限，留待必要时再加。
- **滚动弹幕车道饱和后堆叠**：要"修"就得丢弃超出的弹幕（丢数据）或允许重叠（更难看）。
  现状"退化到最空闲车道"是合理降级，与主流播放器行为一致。
...
我直接抓了 yt-dlp 的 `yt_dlp/extractor/bilibili.py` 源码逐行核对（不再依赖
拿不到报告的成员），发现我们一条错误码文案是**错的**：

| 码 | 我们原来的文案 | yt-dlp 的真实含义 |
|---|---|---|
| `-401` | 「非法访问（URL 缺少必填字段或内容不可用），请回到视频页重新点一次下载」 | `Request is blocked by server (401), please wait and try later` —— **被服务器拦截/限流**，应等待重试 |

区别不是措辞，是**排查方向**：原文案会让用户去检查 URL 和视频页；
真实原因是被限流，正确动作是等一会儿再试。已改为「请求被服务器拦截（401），
请等待片刻后重试」。

### 补充 yt-dlp 有而我们没有的码

- `10004001` 地区限制
- `10004004` / `10004005` / `10023006` 需要登录

### 同时确认无误的（对照结论，不改）

- `11010`（合集/列表失效）—— 与我们 v1.4.16 加的一致
- `-352` —— yt-dlp 也是「Request is rejected by server (352)」
- `v_voucher` —— yt-dlp 明确当作**限流信号**处理（与我们 v1.4.9 的认知一致，
  再次印证它不是参数缺失）

## [1.4.18] - 2026-09-19

### HEVC 兼容性：合并产物补 hvc1 标签

B 站部分 HEVC 流的 stsd sample entry 写的是 `hev1`，而 QuickTime / Safari /
iOS 相册**只认 `hvc1`** —— 合并产物在这些播放器里无法解码（画面不出来，
但文件看起来完全正常）。

修复（`src/core/mp4.js`）：`buildMergedMoov` 里把视频 trak 的 `hev1` 4CC
替换为 `hvc1`。**视频字节本身不动**，只改 sample entry 的标签
（与 mp4box / ffmpeg 的默认行为一致）。

实测（合成夹具）：产物含 `hvc1`、不再含 `hev1`。

> **未真机验证**：真实 B 站 HEVC 流是否用 `hev1` 尚未确认。若实际已经是
> `hvc1`，这个替换就是 no-op，不会有任何副作用。

### 本轮外部挖取的最终状态说明（重要）

派了三路成员并行深挖外部仓库，但**只有 qa-lead 的 mp4 审计回传了完整正文**；
另外两路（investigator 的 yt-dlp/yutto 对比、product-reviewer 的功能矩阵）
多次只回完成通知、没有正文，催过也无果。

因此本轮落地的内容**全部有本地实测或逐行核依据**：
- 片段扫描大 moof 被静默丢弃 —— 合成夹具实测证实前提（moof 20088 > 16384 窗口）
- durl 多段只下第一段 —— 逐行核对 + 回归测试（产物 = 段1 拼接 段2）
- HEVC `hvc1` —— 合成夹具实测

**未做、也不会假装做过**：与 yt-dlp / yutto 的完整对照。已列为待办。
